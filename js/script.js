...
;(function () {
  ...
  $(function () {
      ...
      $.getScript(
        ...
        function () {
          ...
          for (let i = 0; i < plugins.maps.length; i++) {
            ...
            let markerItems = plugins.maps[i].querySelectorAll(
    ...
    function scrollText($this) {
      var translate =
        ((scrollTop - $($this).data('orig-offset')) / $window.height()) * 35
      $($this).css({
    ...
    function initNinaButtons(ninaButtons) {
      for (var i = 0; i < ninaButtons.length; i++) {
        ...
        var textNode = Array.prototype.slice
          .call(ninaButtons[i].childNodes)
        ...
        dummy.outerHTML = textNode.textContent
          .split('')
          .map(function (letter) {
        ...
        btn.innerHTML +=
          "<span class='button-original-content'>" + origContent + '</span>'
        var delay = 0.03 * (btn.childElementCount - 1)
        btn.getElementsByClassName(
          'button-original-content'
        )[0].style.transition =
          'background .22s, color .22s, transform .22s ' + delay + 's'
      ...
      for (i = 0; i < plugins.countDown.length; i++) {
        var countDownItem = plugins.countDown[i],
          ...
          format = countDownItem.getAttribute('data-format')
            ? countDownItem.getAttribute('data-format')
            : 'YYYY/MM/DD hh:mm:ss',
          expiryText = countDownItem.getAttribute('data-expiry-text')
            ? countDownItem.getAttribute('data-expiry-text')
            : 'Countdown finished',
          labels = countDownItem.getAttribute('data-labels')
            ? countDownItem.getAttribute('data-labels')
            : '',
          layout = countDownItem.getAttribute('data-layout')
            ? countDownItem.getAttribute('data-layout')
            : '{yn} {yl} {on} {ol} {dn} {dl} {hnn}{sep}{mnn}{sep}{snn}',
          settings = []
        if (labels.length > 0) {
    ...
    if (plugins.styleSwitcher.length) {
      for (i = 0; i < plugins.styleSwitcher.length; i++) {
        var $currentSwitcher = $(plugins.styleSwitcher[i]),
          $switcherContainer,
          $switcherSection = $(
            $currentSwitcher.find('> .style-switcher-container > .section')[0]
          ),
          $switcherPanel = $(
            $currentSwitcher.find(
              '> .style-switcher-panel-wrap .style-switcher-panel'
            )[0]
          ),
          $switcherToggle = $(
            $currentSwitcher.find('.style-switcher-toggle')[0]
          )
        if ($switcherToggle) {
        ...
        if ($currentSwitcher.attr('data-container')) {
          $switcherContainer = $(
            $currentSwitcher.find($currentSwitcher.attr('data-container'))
          )
        } else {
        ...
        $switcherPanel.find('li > .button').click(
          (function ($switcherContainer, $activeButton) {
            var currentClassSet = '',
          ...
          .slick({
            ...
            prevArrow:
              $slickItem.attr('data-custom-arrows') === 'true'
                ? $('.slick-prev[data-slick="' + $slickItem.attr('id') + '"]')
                : '',
            nextArrow:
              $slickItem.attr('data-custom-arrows') === 'true'
                ? $('.slick-next[data-slick="' + $slickItem.attr('id') + '"]')
                : '',
    ...
    if (plugins.dateCountdown.length) {
      for (var i = 0; i < plugins.dateCountdown.length; i++) {
        var node = plugins.dateCountdown[i],
